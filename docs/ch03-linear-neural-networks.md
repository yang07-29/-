...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | 下方白话术语 → 五步训练循环 → 3.1 的 Shape 例子 | 能指出预测、损失、梯度、参数更新分别发生在哪一步 |
| 90 天后复习 | 一句话主线 → 训练闭环图 → 主动回忆 4、8、9、11 题 | 10 分钟内恢复回归和分类的共同训练链 |
| 面试前复习 | 交叉熵为何接 logits、广播错误、梯度累加、数值稳定 | 能用 Shape 或公式说明原因，不只背 API 名 |

**最小记忆集：**

1. 线性回归是 `X @ w + b`，Softmax 回归是为每个类别产生一个线性 logit；
2. 损失衡量当前预测，`backward()` 只算梯度，`step()` 才改参数；
3. 分类交叉熵直接接 logits，框架内部稳定地完成 `log_softmax`；
4. Shape 错误最危险的不是总报错，而是广播后悄悄算出错误的更大张量。

### 专有名词白话表

| 术语 | 白话解释 | 常见代码 |
| --- | --- | --- |
| 特征（feature） | 模型用来作判断的输入数字，例如面积、房龄 | `X` |
| 标签（label） | 希望模型最终猜中的真实答案 | `y` |
| 参数（parameter） | 训练过程中会被修改的权重和偏置 | `model.parameters()` |
| logit | 分类器给每个类别的原始分数，还不是概率 | `model(X)` 的 `(B,C)` 输出 |
| 损失（loss） | 把“预测错多少”压成可优化的数字 | `loss_fn(output, y)` |
| 梯度（gradient） | 当前参数稍微变化时，损失会朝哪个方向、变化多快 | `parameter.grad` |

### 教材高价值问答

<details>
<summary>【机制】为什么 batch 变大时通常使用平均损失，而不是直接把所有样本损失求和？</summary>

平均损失让梯度尺度大致不随 batch 样本数线性增长，因此更容易沿用相近学习率。若用求和，batch 从 32 增到 128 时，在样本梯度分布相近的情况下总梯度约放大 4 倍；同一学习率可能让更新过猛。求和并非数学上错误，但必须同步考虑学习率和梯度累积的缩放口径。

</details>

<details>
<summary>【推导】Softmax 回归已经有多个输出，为什么决策边界仍然是线性的？</summary>

类别 $i$ 胜过类别 $j$ 的条件是 $w_i^Tx+b_i>w_j^Tx+b_j$，移项得到 $(w_i-w_j)^Tx+(b_i-b_j)>0$。边界仍是一条直线、平面或超平面。Softmax 只把多个分数转成概率，不会自动增加非线性；要学习弯曲边界，需要隐藏层和激活函数。

</details>

<details>
<summary>【代码】只执行 `loss.backward()`，模型为什么不会自己变好？</summary>

`backward()` 只沿计算图求导，并把结果累加到参数的 `.grad`；参数数值没有变化。优化器还必须读取这些梯度执行 `step()`。完整证据链是：step 前后参数不同、梯度有限且非 None、训练指标随多步更新改变。漏掉 `zero_grad()` 则会把新旧批次梯度累加。
...
**结论：**MSE 平滑、对大误差惩罚更重；MAE 对离群点更稳健，但在误差为 0 处不可导且梯度大小几乎恒定。**机制：**单样本误差为 $e$ 时，MSE 梯度正比于 $2e$，大误差产生大更新；MAE 的次梯度主要是 $\mathrm{sign}(e)$。**工程影响：**数据含异常值时 MSE 可能被少量样本支配，MAE 又可能在最优点附近来回摆动，可考虑 Huber 损失折中。**误区：**“MAE 一定更好”不成立，损失还隐含噪声分布假设。**追问：**MSE 对应高斯噪声的极大似然，MAE 更接近拉普拉斯噪声假设。
