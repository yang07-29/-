...
> 原创学习笔记，章节顺序参考[《动手学深度学习》官方目录](https://zh-v2.d2l.ai/chapter_recurrent-neural-networks/index.html)
...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | 序列窗口 → 词元化 → 隐状态递推 → 字符预测 | 能解释“当前输入”和“过去摘要”怎样共同产生新状态 |
| 90 天后复习 | 本章地图 → Shape 表 → detach/重置区别 → 困惑度 | 能画出三步 RNN 展开图并标出共享参数 |
| 面试前复习 | BPTT、梯度消失爆炸、采样方式、递归误差、梯度裁剪 | 能从状态链和 Jacobian 连乘解释机制 |

**最小记忆集：**

1. 序列顺序不能随意打乱，标签常是输入向右移动一步；
2. $h_t=f(x_t,h_{t-1};\theta)$ 在所有时间步共享参数 $\theta$；
3. detach 保留状态数值但切断旧梯度，重置则连记忆也清掉；
4. BPTT 的长链连乘会导致梯度消失或爆炸，裁剪只处理爆炸；
5. 单步预测好不代表递归多步好，模型输出会成为新的输入。

### 专有名词白话表

| 术语 | 白话解释 | 常见 Shape/代码 |
| --- | --- | --- |
| 时间步（time step） | 序列中当前处理到的位置 | `t` |
| 词元（token） | 文本切分后交给模型的最小编号单位 | 字符、词、子词 |
| 词表（vocabulary） | token 与整数编号之间的对照表 | `token_to_idx` |
| 隐状态（hidden state） | 模型截至当前时刻保留的历史摘要 | `(B,H)` |
| 自回归（autoregressive） | 用过去真实值或预测值继续预测下一步 | `x[t] -> x[t+1]` |
| 困惑度（perplexity） | 平均负对数似然取指数；越小说明越不犹豫 | `exp(mean_loss)` |
| BPTT | 把 RNN 沿时间展开后做反向传播 | 多步计算图 |
| 梯度裁剪 | 梯度范数太大时整体缩短，避免破坏性更新 | `clip_grad_norm_` |

### 教材高价值问答

<details>
<summary>【数据】随机采样和顺序采样训练 RNN 时，隐状态处理为什么不同？</summary>

随机采样的相邻 batch 通常不是原序列的连续片段，沿用状态会把无关样本历史泄露进来，因此一般重置。顺序采样保持片段相邻，可以把状态数值传给下一批，但要 detach 切断旧计算图，避免反向无限跨批增长。关键不是采样器名字，而是 batch 之间是否保持真实时间连续性。

</details>

<details>
<summary>【计算图】`state.detach()` 为什么既保留记忆，又能截断 BPTT？</summary>

detach 返回数值相同但不再连接旧计算历史的 Tensor。下一批仍从上批最终状态数值开始，所以短期记忆保留；新批建立的新计算图却不会继续追溯更早步骤。它是训练近似，不等于把状态置零，也不等于关闭当前批内梯度。

</details>

<details>
<summary>【指标】困惑度为 20 到底说明什么，为什么不能跨词表随便比较？</summary>

可粗略理解为模型平均每一步像在约 20 个等可能候选中犹豫，数学上是平均交叉熵的指数。不同 token 切分、词表大小和测试语料会改变每一步任务难度，所以只有评价数据和分词口径一致时比较才有意义；困惑度低也不自动保证生成事实正确。
