...
[卷积基础完整代码](../code/ch06/convolution_basics.py) · [2→16 通道扩展示例](../code/ch06/channel_expansion_demo.py) · [LeNet 完整训练代码](../code/ch06/lenet_smoke.py) · [官方章节目录](https://zh-v2.d2l.ai/chapter_convolutional-neural-networks/index.html)
...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | 局部性/共享 → 手算互相关 → padding/stride → 通道 | 能手算一个输出位置并预测整层 Shape |
| 90 天后复习 | 章节地图 → `2→16` 例子 → LeNet Shape 链 | 能从 `(N,C,H,W)` 追踪到分类 logits |
| 面试前复习 | 参数量、感受野、多通道求和、1×1、池化边界 | 回答时能写出 OIHW 权重 Shape 和输出尺寸公式 |

**最小记忆集：**

1. 卷积用局部连接和空间权重共享保留图像结构、减少参数；
2. PyTorch 输入是 NCHW，标准卷积权重是 `(C_out,C_in,K_h,K_w)`；
3. 一个输出通道会对全部输入通道分别互相关再求和；
4. padding 控边界和尺寸，stride 控采样步幅，二者共同决定输出高宽；
5. 池化通常逐通道做固定汇聚，不负责学习跨通道组合。

### 专有名词白话表

| 术语 | 白话解释 | 常见代码/Shape |
| --- | --- | --- |
| 局部连接 | 一个输出先只看附近小窗口，不一开始读取整张图 | `kernel_size` |
| 权重共享 | 同一个检测规则滑到不同空间位置重复使用 | 一份卷积核遍历 H、W |
| 互相关 | 窗口与核对应相乘再求和；深度学习 API 通常实际算它 | `Conv2d` |
| 特征图 | 一个通道的一张二维响应图 | `(H,W)` |
| 感受野 | 某个输出最终可能受到输入多大区域影响 | 堆叠层后逐渐扩大 |
| 填充（padding） | 在边缘补值，让边界也能被计算并控制输出尺寸 | `padding=` |
| 步幅（stride） | 窗口每次移动多少格 | `stride=` |
| 池化（pooling） | 每个小窗口只保留最大值或平均值，压缩空间 | `MaxPool2d` |

### 教材高价值问答

<details>
<summary>【辨析】深度学习里通常计算互相关，为什么层名仍叫卷积？</summary>

严格数学卷积会先翻转核，互相关不翻转。训练中的核本来就是可学习参数，即使采用翻转定义，模型也能学到相应翻转后的权重，因此表达能力没有本质差异。深度学习社区沿用“卷积层”这个名称，代码和 Shape 推导时则按框架实际定义计算。

</details>

<details>
<summary>【通道】输出通道从 6 增加到 16，是不是网络凭空创造了更多信息？</summary>

不是。标准 `Conv2d(6,16,...)` 用 16 套不同权重分别重组同样的 6 个输入通道，得到 16 种可学习描述。表示维度增加给模型更多特征槽位，但这些值仍由输入和参数决定；通道多也不保证每个都独特或有用，训练可能产生冗余通道。

</details>

<details>
<summary>【结构】池化和 stride 卷积都能让高宽变小，它们有什么区别？</summary>

池化通常没有可学习权重，按固定最大值或平均值逐通道汇聚；stride 卷积一边下采样，一边用可学习核做空间与通道变换。前者简单、稳定，后者表达更灵活但增加参数和优化负担。二者都会丢失部分空间细节，不能只凭“输出尺寸相同”认为功能完全相同。
...
### 读者追问：`Conv2d(6,16)` 的 16 个通道怎样读取前面的 6 个通道？

> **包装后的问题：**在标准卷积 `nn.Conv2d(6,16,kernel_size=3)` 中，输出为什么可以从 6 个通道变成 16 个通道？16 个输出通道是每个都读取全部 6 个输入通道，还是各自只读取固定组合？如果是 `2→16`，能否理解为 16 套核、每套包含 2 张二维核？

#### 先给 30 秒回答

默认 `groups=1` 时，**每个输出通道都会读取全部输入通道**。`Conv2d(6,16,3)` 的权重 Shape 是 `(16,6,3,3)`：可以说有 16 套输出核，每套包含 6 张 `3×3` 二维核；同一套里的 6 次卷积结果逐位置相加，再加一个偏置，得到 1 张输出特征图。16 套权重不同，所以得到 16 张不同的输出特征图。

`2→16` 的理解也类似：有 16 套输出核，每套含 2 张二维核，因此总共能数出 `16×2=32` 张二维核切片。不过不要说“16 个 group”，因为 `groups` 是 PyTorch 分组卷积的专门参数；更准确的说法是“16 个输出核（filter banks），每个核的输入深度为 2”。

#### 为什么“少输入”能得到“多输出”？

输出通道不是把输入复制 16 份，而是用 16 套不同的可学习权重，对同一批输入做 16 种不同组合。对于 `2→16` 的 `1×1` 卷积，在某个像素位置可写成：

$$
\begin{aligned}
y_0 &= w_{0,0}x_0+w_{0,1}x_1+b_0,\\
y_1 &= w_{1,0}x_0+w_{1,1}x_1+b_1,\\
&\ \vdots\\
y_{15} &= w_{15,0}x_0+w_{15,1}x_1+b_{15}.
\end{aligned}
$$

16 行使用不同权重，因而能学习 16 种特征组合。输出通道多，表示模型允许下一层在同一空间位置保存更多种描述；这不表示凭空增加了原始信息，也不保证 16 个通道一定学成完全不同的特征。

#### 可手算的 `2→16` 例子

- **小输入**：一个像素有两个通道，`x=[10,20]`。
- **权重设计**：第 $o$ 个输出通道使用 `[(o+1),1]`，暂时关闭偏置。
- **逐步过程**：`y_o=10×(o+1)+20×1`。
- **具体输出**：16 个通道依次得到 `[30,40,50,...,180]`。

前两个输出可以手算为：

$$
y_0=10\times1+20\times1=30,
\qquad
y_1=10\times2+20\times1=40.
$$

这说明同一个 `[10,20]` 没有被简单复制，而是被 16 行不同的权重分别组合。若核改为 `3×3`，逻辑不变，只是每个输入通道不再贡献一个数，而是贡献一次局部 `3×3` 互相关结果。

#### 对照 Shape 和参数量

| 配置 | 权重 Shape | 每个输出读取几个输入通道 | 二维核切片数 | 含偏置参数量 |
| --- | --- | ---: | ---: | ---: |
| `Conv2d(2,16,1)` | `(16,2,1,1)` | 2 | 32 | `16×2×1×1+16=48` |
| `Conv2d(6,16,3)` | `(16,6,3,3)` | 6 | 96 | `16×6×3×3+16=880` |
| `Conv2d(6,16,3,groups=2)` | `(16,3,3,3)` | 3 | 48 | `16×3×3×3+16=448` |

最后一行才是“只读取固定组合”：`groups=2` 把 6 个输入通道分成两组，每个输出通道只连接所属组的 3 个输入通道。标准卷积默认 `groups=1`，不存在这种分组限制。

#### 用 PyTorch 验证，而不是只背 Shape

[打开完整可运行示例 `channel_expansion_demo.py`](../code/ch06/channel_expansion_demo.py)。它会固定权重并打印：

```text
输入 Shape: (1, 2, 1, 1)
权重 Shape: (16, 2, 1, 1)
输出 Shape: (1, 16, 1, 1)
16 个输出值: [30.0, 40.0, 50.0, ..., 180.0]
标准 6→16 权重 Shape: (16, 6, 3, 3)
标准 6→16 含偏置参数量: 880
groups=2 时权重 Shape: (16, 3, 3, 3)
```

**最容易混淆的地方：**“一个输出核包含 6 张二维核切片”和“有 6 个独立输出”不是一回事。前者的 6 个结果会先求和，只形成一个输出通道；换另一整套权重，才形成下一个输出通道。
...
### 程序二：把 `2→16` 的 16 个输出全部打印出来

[打开 `channel_expansion_demo.py`](../code/ch06/channel_expansion_demo.py)

这份程序不训练模型，而是人为固定 `1×1` 卷积的 16 套权重，使每个输出都能手算。阅读时依次核对输入 `(1,2,1,1)`、权重 `(16,2,1,1)` 和输出 `(1,16,1,1)`，再比较标准卷积与 `groups=2` 的权重 Shape。
...
<details>
<summary>22. 【八股深答·读者追问】`Conv2d(6,16,3)` 中，每个输出通道怎样取得前面 6 个通道的信息？</summary>

**结论：**默认 `groups=1` 时，每个输出通道都读取全部 6 个输入通道。权重 Shape 为 `(16,6,3,3)`，即 16 套输出核，每套含 6 张 `3×3` 二维核切片。

**推导：**第 $o$ 个输出为 $Y_o=\sum_{c=1}^{6}X_c\star K_{o,c}+b_o$。同一套里的 6 个结果先逐位置相加，只产生一张 $Y_o$；更换 $o$ 对应的整套权重，才产生下一输出通道。因此共有 `16×6=96` 张二维核切片，含偏置参数量为 `16×6×3×3+16=880`。

**工程影响：**输入 `(N,6,H,W)` 会得到 `(N,16,H_out,W_out)`。排查 Shape 时直接打印 `conv.weight.shape`；如果看到 `(16,3,3,3)`，还要检查是否设置了 `groups=2`。

**边界与误区：**可以口语化地说“16 套核，每套 6 张”，但不要说“16 个 group”。`groups` 是分组连接参数；只有 `groups>1` 时，每个输出才只读取固定的一部分输入通道。输出通道变多是更多种可学习组合，不是创造了额外原始信息。

**面试追问：**若改成 `Conv2d(2,16,1)`，权重 Shape 是 `(16,2,1,1)`；可理解为 16 套核、每套 2 个通道权重，共 32 个权重，若有偏置再加 16 个参数。
