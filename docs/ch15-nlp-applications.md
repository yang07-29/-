...
> 章节顺序与小节名称参考[《动手学深度学习》中文 2.0 官方目录](https://zh-v2.d2l.ai/chapter_natural-language-processing-applications/index.html)
...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | 输出粒度表 → 情感分类 → 文本对 NLI → BERT 任务头 | 能先判断一个序列一个标签还是每个 token 一个标签 |
| 90 天后复习 | BiRNN/textCNN 对比 → 对齐/比较/汇总图 → 微调流程 | 能恢复“骨干表示+小任务头”的主线 |
| 面试前复习 | padding/mask、双向边界、卷积窗口、NLI 偏差、分层学习率 | 能解释模型为什么可能利用数据捷径 |

**最小记忆集：**

1. 先确定任务输出粒度，再决定怎样汇聚 token 表示；
2. BiRNN 顺序递归看双向上下文，textCNN 并行捕获局部 n-gram；
3. NLI 是有方向的文本对关系，交换前提和假设可能改变标签；
4. BERT 用同一骨干支持序列级和词元级任务，只更换读取位置与任务头；
5. 微调学习率通常小于从零训练，预训练好不等于下游一定好。

### 专有名词白话表

| 术语 | 白话解释 | 输出 |
| --- | --- | --- |
| 情感分析 | 判断一段文本表达正面、负面或其他情绪 | `(B,C)` |
| 自然语言推断（NLI） | 判断假设是否被前提支持、反驳或无法确定 | 三分类 |
| 序列级任务 | 整段输入只给一个答案 | 分类 logits |
| 词元级任务 | 每个有效 token 都要给一个答案 | `(B,S,C)` |
| 文本卷积 | 用固定宽度窗口扫描相邻 token 模式 | textCNN |
| 对齐（alignment） | 为一个句子的词寻找另一句中相关的词 | 注意力权重 |
| 任务头（task head） | 接在预训练骨干后、负责输出具体任务答案的小层 | Linear 等 |
| 微调 | 在下游标注数据上继续更新预训练模型 | 小学习率训练 |

### 教材高价值问答

<details>
<summary>【比较】BiRNN 和 textCNN 做情感分类，各自更擅长什么？</summary>

BiRNN 按顺序递归整合双向上下文，更自然地保留远近顺序关系，但时间并行度较低；textCNN 用多种窗口抓局部 n-gram，再全局池化，计算并行且对关键词位置较稳健。不能只凭模型名判断，需在相同词向量、预算和数据划分下比较。

</details>

<details>
<summary>【读取】为什么 BERT 分类常读取 `<cls>`，词元标注却读取每个位置？</summary>

序列分类需要把整段压成一个向量，预训练和微调约定用 `<cls>` 位置汇总后接分类头；词元标注要求每个 token 各自输出标签，因此保留 `(B,S,H)` 全序列表示并逐位置接同一个 Linear。任务输出粒度决定读取位置，不是 BERT 只能使用 `<cls>`。

</details>

<details>
<summary>【迁移】预训练模型很强，为什么小数据微调仍可能失败？</summary>

下游标签可能噪声大、分布与预训练差异大，任务头或学习率也可能不合适；小数据还容易让模型抓住长度、否定词或模板等捷径。应使用验证集、分层学习率、早停、困难样例和反事实检查，不能把训练准确率高直接当成语义理解。
...
原有 #139 之外，新增 [#438 找到字符串中所有字母异位词](https://leetcode.cn/problems/find-all-anagrams-in-a-string/)，练固定长度滑动窗口和频次增量维护。解析见[新增题完整解析](leetcode-hot100-expanded-practice.md#第-15-章固定长度滑动窗口)，代码见 [hot100_find_all_anagrams.py](../code/ch15/hot100_find_all_anagrams.py)。
...
<details>
<summary>21. 【八股深答】BiRNN 与 textCNN 做文本分类时各有什么归纳偏置？</summary>

**结论：**BiRNN 按顺序递归汇聚双向上下文，textCNN 用不同宽度卷积捕获局部 n-gram 并池化。**机制：**前者显式保留时序状态，后者对局部模式位置更稳健且可高度并行。**工程影响：**长依赖、延迟、并行度和数据规模会影响选型，应在相同词向量和预算下比较。**误区：**双向模型不能直接用于需要因果生成的下一词预测，因为它读取未来。**追问：**Transformer 自注意力提供全局交互，但计算和数据需求也不同。

</details>

<details>
<summary>22. 【八股深答】微调预训练模型时为什么常给 backbone 更小学习率？</summary>

**结论：**预训练参数已包含通用表示，大步更新容易破坏；随机初始化任务头则需要更快适应。**机制：**分层学习率在保留旧知识与拟合新任务之间折中。**工程影响：**优化器用参数组设置不同 lr，配合 warmup、权重衰减和验证集早停；小数据时尤其关注过拟合。**误区：**冻结 backbone 并不总优于微调，也不能默认所有层使用同一最优学习率。**追问：**逐层衰减学习率是更细的扩展，但应通过消融验证。

</details>

<details>
<summary>23. 【八股深答】自然语言推断为何不能只靠两个句子的词重合度？</summary>

**结论：**蕴含、矛盾和中立取决于语义关系、否定、数量、实体和方向，词重合只是脆弱线索。**机制：**可分解注意力等模型先做跨句对齐，再比较对应成分并汇总判断。**工程影响：**应检查困难样例和混淆矩阵，防止模型利用数据集中的表面偏差。**误区：**注意力对齐高不等于逻辑蕴含，更不构成因果解释。**追问：**交换前提与假设通常会改变标签，说明关系具有方向性。
