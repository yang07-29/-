...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | 激活函数白话 → “没有激活仍是线性”推导 → 过拟合曲线 | 能解释 MLP 为什么比线性模型多了什么能力 |
| 90 天后复习 | 最小记忆集 → 4.4～4.8 的诊断表 → 章末主动回忆 | 能从训练/验证曲线判断下一步该查什么 |
| 面试前复习 | ReLU、Dropout、L1/L2/AdamW、梯度消失爆炸、分布偏移 | 回答包含机制、训练/评估差异和适用边界 |

**最小记忆集：**

1. 多层 Linear 没有激活仍等价于一层，非线性才真正扩大函数类别；
2. 欠拟合是模型连训练集都学不好，过拟合是训练好但验证差；
3. 权重衰减和 Dropout 都是约束模型，但作用位置、训练行为不同；
4. `train()`/`eval()` 管模块行为，`no_grad()`/`inference_mode()` 管是否记录梯度；
5. 训练分布、标签规律或输入含义变化时，离线高分可能失效。

### 专有名词白话表

| 术语 | 白话解释 | 常见代码/现象 |
| --- | --- | --- |
| 隐藏层（hidden layer） | 不直接给最终答案，先把输入组合成新的中间特征 | `Linear → ReLU` |
| 激活函数 | 在层之间加入不能被合并掉的非线性变换 | `ReLU()` |
| 模型容量（capacity） | 模型能表达多复杂规律的能力，不等同于一定学得好 | 深度、宽度、参数量 |
| 欠拟合 | 训练集也表现差，说明表达、优化或数据链路不足 | 训练/验证误差都高 |
| 过拟合 | 训练集记得很好，换到验证数据明显变差 | 训练误差低、验证误差升高 |
| 正则化 | 主动限制模型只靠某些复杂记忆取胜 | weight decay、Dropout、增广 |
| 分布偏移 | 部署时看到的数据规律与训练时不同 | 输入统计或标签关系改变 |

### 教材高价值问答

<details>
<summary>【辨析】网络更宽、更深，为什么不保证测试结果更好？</summary>

宽和深增加可表示的函数，但训练还受优化难度、数据量、噪声和正则化影响。容量太小会欠拟合，容量过大又可能记住训练细节；即使训练损失更低，验证误差也可能更高。因此比较架构必须固定数据划分和训练预算，并同时看训练、验证曲线，而不是只数参数。

</details>

<details>
<summary>【机制】Dropout 为什么训练时随机丢弃，预测时却全部保留？</summary>

倒置 Dropout 在训练时使用 $m\odot h/(1-p)$，其期望仍是 $h$；预测时关闭随机掩码即可保持相近尺度并获得确定输出。它通过随机破坏固定共适应来正则化。`no_grad()` 只关闭计算图，不会关闭 Dropout，所以验证必须同时调用 `eval()`。

</details>

<details>
<summary>【数据】为什么验证集能反复选模型，而测试集不应该参与调参？</summary>

每次根据某个数据集的结果修改超参数，都会把该数据集的信息间接写入决策。验证集就是为这种迭代选择准备的；测试集应尽量只在方案冻结后使用，估计最终泛化。若反复看测试结果再改模型，测试集事实上已经变成验证集，报告会过于乐观。
...
<details>
<summary>21. 【八股深答】为什么多层线性层之间必须有非线性激活？</summary>

**结论：**没有激活时，任意多层仿射变换仍可合并成一次仿射变换，深度没有增加函数类别。**推导：**$W_2(W_1x+b_1)+b_2=(W_2W_1)x+(W_2b_1+b_2)$。**工程影响：**检查网络定义时要确认激活真的位于层之间，且 forward 没有漏调用。**误区：**参数更多不等于表达能力必然更强；纯线性堆叠可能有不同优化参数化，但决策边界仍是线性的。**追问：**输出层是否需要激活取决于损失接口，例如交叉熵通常直接接 logits。
...
<details>
<summary>23. 【八股深答】L1、L2 与 AdamW 的区别应怎样回答？</summary>

**结论：**L1 倾向稀疏，L2 对大权重施加连续惩罚；AdamW 将权重衰减与自适应梯度更新解耦。**机制：**L1 的次梯度近似常数符号，L2 梯度与权重成正比；在普通 SGD 中 L2 常与衰减等价，但在 Adam 的坐标自适应缩放下不再简单等价。**工程影响：**使用 Adam 系优化器时通常明确选择 `AdamW(weight_decay=...)`，并常排除 bias/归一化参数。**误区：**权重衰减不是学习率的替代品，也不保证所有任务泛化都提升。**追问：**为什么 batch norm 的缩放参数常不衰减？因为其尺度语义与普通权重不同，需实验判断。
