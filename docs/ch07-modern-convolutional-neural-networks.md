...
下面这些词会反复出现。先知道它们“在做什么”，再记英文和公式：

| 专有名词 | 白话解释 | 在代码里通常看到什么 |
| --- | --- | --- |
| 架构（architecture） | 网络的整体搭法：哪些层先做、哪些层后做、数据怎样走 | 一个完整的 `nn.Module` |
| 块（block） | 把经常一起出现的几层打包成一块积木，之后重复使用 | 返回 `nn.Sequential` 的函数或一个小类 |
| 表征（representation） | 模型把原始图片换成一组更方便判断的数字 | 中间层输出 Tensor |
| 特征图（feature map） | 一个通道对应的一张二维响应图：哪里像某种模式，哪里数值就可能大 | `(H,W)`；批量中整体是 `(N,C,H,W)` |
| 通道（channel） | 同一个位置的不同描述，例如“像竖边”“像纹理”“像某个部件” | NCHW 中的 `C` |
| 非线性（nonlinearity） | 让多层不能被合并成一次线性计算，模型才能表示弯曲、分段的复杂规则 | `ReLU` 等激活函数 |
| 感受野（receptive field） | 某个输出值最终能够受到输入图像多大区域影响 | 由核大小、层数、步幅共同决定 |
| 全局平均池化（GAP） | 每个通道整张图求一个平均数，只消掉高和宽，不混掉通道 | `AdaptiveAvgPool2d((1,1))` |
| 降维/瓶颈（bottleneck） | 先用 `1×1` 把通道压少，再做昂贵卷积，像先把要处理的材料变少 | `Conv2d(C_in,C_mid,1)` |
| 分支（branch） | 同一个输入同时走几条不同路线，最后再合并 | Inception 中多个并行子模块 |
| 残差/捷径（residual/shortcut） | 主路只学“需要改多少”，原输入另走近路直接加回来 | `y = block(x) + x` |
| 拼接（concatenation） | 把几组特征沿通道方向排在一起，旧特征都保留 | `torch.cat(..., dim=1)` |

后文仍会使用标准术语，因为看论文、代码和面试都需要；但第一次出现关键术语时，会尽量紧跟一句白话说明。
...
> **课程编号提示：**你截图中的视频把 NiN 标成“7.4 课”，本仓库按《动手学深度学习》2.0 官方书面目录编号，因此 NiN 位于 7.3，7.4 是 GoogLeNet。知识内容相同，只是视频切分编号与书面小节编号不同。
...
| 比较项 | AlexNet | VGG | NiN |
| --- | --- | --- | --- |
| 卷积部分 | 大核开头，后接多层卷积 | 重复堆叠规则的 `3×3` 卷积块 | 普通空间卷积后，再接两个 `1×1` 卷积 |
| 白话理解 | 先用较大窗口快速看较大范围 | 用很多相同小积木逐层加深 | 先看邻域，再在每个像素位置上用小 MLP 重组通道 |
| 分类部分 | 展平后使用大型全连接层 | 展平后使用大型全连接层 | 类别通道加全局平均池化 |
| 参数集中位置 | 全连接头通常很重 | 全连接头通常也很重 | 去掉大型全连接头，参数明显减少 |
| 输入尺寸依赖 | 全连接层通常要求固定展平长度 | 同样通常要求固定展平长度 | GAP 能把任意高宽压成 `1×1`，结构上更灵活 |
| 过拟合直觉 | 大型全连接头容量很高，较容易记训练细节 | 同样需要 Dropout 等手段 | 参数更少、结构约束更强，通常不那么容易过拟合 |
| 必须记住的边界 | 历史意义大，不是现代任务固定模板 | 结构规则，但计算和存储仍重 | 参数少不等于一定更准，也不等于训练一定更快 |

截图中的“NiN 使用全局平均池化替代 VGG 和 AlexNet 的全连接层”，准确地说是：最后先让通道数等于类别数，再让每个类别通道在全部空间位置求平均，得到该类别的一个 logit。

```mermaid
flowchart LR
    A["AlexNet / VGG 特征<br/>N×C×H×W"] --> B["Flatten<br/>N×(C·H·W)"]
    B --> C["大型全连接头"]
    C --> D["N×类别数"]
    E["NiN 特征<br/>N×C×H×W"] --> F["1×1 卷积映射到类别通道<br/>N×类别数×H×W"]
    F --> G["每个通道整张图求平均"]
    G --> H["N×类别数"]
```

### 可计算例子：为什么说 NiN 的分类头参数少

假设送到分类头的特征是 `(N,64,7,7)`，类别数为 10：

- **VGG 式小型全连接头**：先展平为 `64×7×7=3136`，再做 `3136→128→10`；
- **NiN 式分类头**：用 `1×1` 卷积完成 `64→10`，再对每个类别通道做全局平均。

参数量分别为：

$$
\begin{aligned}
P_{FC} &= 3136\times128+128+128\times10+10=402826,\\
P_{NiN\ head} &= 64\times10+10=650.
\end{aligned}
$$

这个数字只用于隔离比较“分类头”，不是说完整 NiN 只有 650 个参数。NiN 前面的空间卷积和 `1×1` 卷积仍然有参数、计算量和激活内存。

[运行 `nin_vs_vgg_head_demo.py`](../code/ch07/nin_vs_vgg_head_demo.py) 可以看到两种头都把 `(2,64,7,7)` 变成 `(2,10)`；输入高宽换成 `9×9` 后，GAP 头仍能输出 `(2,10)`，固定 `Linear(3136,...)` 则会因展平长度变化而不匹配。
...
**短答：**普通卷积先读取邻域，后面的两个 `1×1` 卷积在每个位置上继续重组通道；每层后面的 ReLU 又增加一次非线性，所以它像一个在所有像素位置共享的小型多层感知机。

**白话例子：**普通卷积先判断“这一小块有哪些初级线索”，第一个 `1×1` 学习怎样搭配这些线索，第二个再把上一层组合继续组合。若删除一层，模型仍能运行，但逐位置变换更浅、表达能力和参数量都会下降，最终效果要靠实验判断。

</details>

<details>
<summary>2. 如果两个 `1×1` 卷积之间没有 ReLU，堆两层还有同样意义吗？</summary>

没有。忽略偏置写成 $W_2(W_1x)$，可合并为 $(W_2W_1)x$，仍然只是一层线性变换。真正让“多层”增加函数表达能力的是中间的非线性激活。两层线性参数化有时可能影响优化，但不能因此说它获得了普通意义上的多层非线性表达。

</details>

<details>
<summary>3. `1×1` 卷积不看相邻像素，为什么还说它有用？</summary>

它不负责扩大空间感受野，而是负责混合同一位置的全部通道。假设某位置有“竖边、红色、纹理”三个响应，`1×1` 可以学出“同时有竖边和纹理时增强”的新通道。邻域信息已经由前面的普通卷积读入，因此两者分工是“空间卷积看附近，`1×1` 在当前位置重新配方”。

</details>

<details>
<summary>4. 全局平均池化没有参数，怎么完成分类？</summary>

真正把特征映射到类别的是它前面的卷积：最后一层先产生 `类别数` 个通道，每个通道可理解为该类别在各位置的证据图。GAP 只把每张证据图的所有位置取平均，得到一个类别 logit。它没有学习参数，但仍处在可求导计算图中，梯度会平均分回各位置并更新前面的卷积权重。

</details>

<details>
<summary>5. 为什么去掉全连接层通常不容易过拟合？能说“一定不会过拟合”吗？</summary>

大型全连接层把每个空间位置连接到大量隐藏单元，参数很多，比较容易记住训练样本的偶然细节。GAP 没有可训练参数，并强制每个类别通道汇总整张图，因此容量更受约束。但完整 NiN 仍可能因数据少、标签噪声、训练太久或前面网络过强而过拟合，所以只能说风险通常降低，不能说一定不会。

</details>

<details>
<summary>6. 参数更少，为什么 NiN 训练不一定更快？</summary>

参数量回答“要存多少权重”，运行时间还取决于“每个权重在多少空间位置被使用”、中间激活大小、算子实现和内存搬运。`1×1` 卷积虽然单个核很小，却要在每个像素位置计算，而且 NiN 块多了额外层。因此参数少通常省模型存储，却不自动推出 FLOPs 更少或实际延迟更低。

</details>

<details>
<summary>7. 为什么不直接把 `384×5×5` 一次映射成 `10×5×5`，然后立刻 GAP？</summary>

这样当然能运行，但会把 384 维特征突然压到 10 个类别通道，形成很窄的信息瓶颈。NiN 在空间卷积后加入多个带 ReLU 的 `1×1` 层，让高维特征先经过更充分的逐位置组合，再形成类别证据。是否真的需要两层以及中间多宽，仍属于要用验证集和消融实验回答的设计问题。

</details>

<details>
<summary>8. GAP 是否意味着 NiN 可以随便换输入分辨率？</summary>

从分类头 Shape 看，GAP 确实能把任意 `H×W` 变成 `1×1`，不会像固定 `Linear(C×H×W,...)` 那样因展平长度改变而报错。但输入太小可能被多次池化压没，输入尺度变化也会改变物体大小、计算量和训练分布。因此是“分类头结构更灵活”，不是“任意尺寸都同样有效”。
...
### 专项程序：VGG 式全连接头和 NiN 式 GAP 头

[打开 `nin_vs_vgg_head_demo.py`](../code/ch07/nin_vs_vgg_head_demo.py)

这份程序把比较范围限定在分类头，打印两者参数量、输出 Shape，并额外测试输入高宽从 `7×7` 改成 `9×9`。它帮助验证三件事：GAP 不新增参数、NiN 头参数明显更少、固定全连接头依赖展平长度。
