...
> 原创学习笔记，正式小节对照[《动手学深度学习》中文 2.0 官方目录](https://zh-v2.d2l.ai/chapter_optimization/index.html)
...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | 一维梯度下降 → SGD 噪声 → Momentum → RMSProp → Adam | 能把每个状态量翻译成“方向”或“尺度” |
| 90 天后复习 | 优化器家族图 → 两个手算更新 → scheduler | 能解释各算法多保存了什么历史 |
| 面试前复习 | 凸性边界、batch/lr、偏差修正、AdamW、恢复状态 | 能说明“训练快”和“泛化好”不是同一问题 |

**最小记忆集：**

1. 梯度给局部方向，学习率决定步长，二者任何一个异常都可能不收敛；
2. mini-batch 在梯度噪声和硬件吞吐之间折中；
3. Momentum 平滑方向，RMSProp 缩放坐标，Adam 组合一阶矩和二阶矩；
4. Adam 的二阶矩不是 Hessian，AdamW 的衰减与自适应梯度解耦；
5. 恢复训练必须恢复优化器和 scheduler 状态。
...
| --- | --- | --- |
| 学习率 | 每次愿意沿建议方向走多远 | `lr` |
| 随机梯度 | 用部分样本估计全数据梯度，有噪声但便宜 | mini-batch `.grad` |
| 动量 | 把近期方向做平滑累积，减少来回摆动 | 一阶状态 `v` |
| 二阶矩 | 梯度平方的移动平均，表示各坐标近期尺度 | Adam/RMSProp 的 `s` |
| 偏差修正 | 修正移动平均初期从 0 开始导致的偏小 | Adam 的 hat 量 |
| 权重衰减 | 每步主动把权重往 0 拉一点 | `weight_decay` |
| 调度器 | 随训练进度改变学习率 | `scheduler.step()` |
| 收敛 | 指标逐渐稳定，不代表一定到全局最优或泛化最好 | loss/metric 曲线 |

### 教材高价值问答

<details>
<summary>【随机性】SGD 某一步可能让损失上升，为什么仍然能训练？</summary>

单个样本或小批量梯度只是全数据梯度的有噪估计，不保证每一步下降；在均匀采样和适当条件下，它的期望方向可接近真实梯度。噪声让更新便宜，也可能帮助离开狭窄区域，但学习率过大时同样会发散，因此应看一段窗口趋势而非要求每步单调。

</details>

<details>
<summary>【比较】Adam 收敛快，为什么很多任务仍会尝试 SGD 或 AdamW？</summary>

Adam 的自适应缩放常让早期训练更省调参，但最终泛化、显存和最优超参数依任务而定。SGD 加动量在部分视觉任务上可能获得更好泛化；AdamW 通过解耦权重衰减改善 Adam 中 L2 与预条件耦合的问题。不存在只凭优化器名称决定胜负的规则。

</details>

<details>
<summary>【状态】为什么只恢复模型参数后，Adam 可能像“重新热身”？</summary>

Adam 的更新依赖历史一阶矩、二阶矩和步数；只恢复模型会把这些状态清零，偏差修正和坐标尺度重新开始，后续轨迹不再连续。无缝续训还要恢复 optimizer、scheduler、scaler 和进度，并确认调用顺序与原训练一致。
