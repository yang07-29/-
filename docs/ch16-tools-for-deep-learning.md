...
> 原创学习笔记，章节顺序参考[《动手学深度学习》官方目录](https://zh-v2.d2l.ai/chapter_appendix-tools-for-deep-learning/index.html)
...
| 场景 | 先看什么 | 达标标准 |
| --- | --- | --- |
| 新手第一次学 | Notebook 状态 → 环境诊断 → checkpoint → GPU 计时 | 能区分代码文件、运行内核和持久化产物 |
| 90 天后复习 | 可复现清单 → OOM 排错 → 新进程恢复验证 | 能在换机器后重建并核对同一实验 |
| 面试前复习 | 随机种子边界、显存构成、异步计时、最小复现 | 回答包含可验证步骤而不是只报名令 |

**最小记忆集：**

1. Notebook 文件不等于当前内核状态，交付前必须重启并全部运行；
2. 环境、代码、数据、配置和随机性都属于实验输入；
3. 日志负责观察，checkpoint 负责恢复，两者不能互相替代；
4. 训练显存包括参数、梯度、优化器状态、激活和临时空间；
5. “成功保存”不算验收，新进程实际加载并运行才算。

### 专有名词白话表

| 术语 | 白话解释 | 验证方式 |
| --- | --- | --- |
| 内核（Notebook kernel） | 真正保存当前变量、导入和执行状态的进程 | 重启后变量消失 |
| 环境（environment） | Python、库、驱动和系统依赖的组合 | 版本清单+最小前向 |
| 持久化存储 | 计算实例释放后仍会保留的文件位置 | 重启/换机检查 |
| checkpoint | 能恢复模型或训练进度的状态包 | 新进程 load |
| 可复现性 | 在明确条件和容差下重新得到相近结果 | 固定配置并重复运行 |
| 最小复现 | 删除无关部分后仍能稳定触发问题的小程序 | 独立脚本 |
| 显存峰值 | 运行过程中同时占用显存最多的时刻 | memory stats |
| 断线恢复 | 连接中断后进程与产物仍可继续使用 | 模拟断线与重连 |

### 教材高价值问答

<details>
<summary>【复现】固定随机种子为什么仍可能得到不同结果？</summary>

种子只控制被覆盖的随机数生成器；并行调度、非确定性算子、浮点归约顺序、驱动和库版本仍会制造差异。应记录环境与数据版本、设置需要的确定性选项，并区分逐位一致、指标容差内一致和结论可重复三种目标。

</details>

<details>
<summary>【恢复】为什么 checkpoint 文件写成功，还不能证明真的能恢复？</summary>

文件可能缺键、损坏、版本不兼容，或只保存模型却漏掉优化器。验收必须在新模型甚至新进程中实际加载，检查 missing/unexpected keys，用固定输入比较预测，并在需要续训时再走一个优化步骤。写文件只是动作，恢复测试才是证据。

</details>

<details>
<summary>【Notebook】为什么“我刚才明明运行成功”仍可能无法交付？</summary>

单元格可以乱序执行，当前变量可能来自已经修改或删除的旧代码，输出也只是过去快照。保存 `.ipynb` 不会保存一条可靠的状态构建历史。交付前应重启内核、从上到下全部运行，并把路径、依赖和随机配置显式化。
...
本章频繁处理数据、日志与 checkpoint 路径，因此加入 [#71 简化路径](https://leetcode.cn/problems/simplify-path/) 练栈式状态处理。它不代表云平台本身依赖这道算法，只是把常见路径规则变成可手推的状态机。解析见[新增题完整解析](leetcode-hot100-expanded-practice.md#第-16-章栈与工程路径)，完整代码见 [hot100_simplify_path.py](../code/ch16/hot100_simplify_path.py)。
...
<details>
<summary>19. 【八股深答】固定随机种子为什么仍不能保证完全复现？</summary>

**结论：**种子只控制已覆盖的随机数序列，硬件、并行调度、非确定性算子、库版本和数据顺序仍可造成差异。**机制：**浮点加法不满足结合律，并行归约顺序变化就可能产生微小误差，之后被训练放大。**工程影响：**记录代码/数据/依赖/硬件版本，设置确定性选项，保存划分与采样状态，并报告容差而非只报一个种子。**误区：**同 seed 不等于同结果，完全确定性也可能牺牲性能。**追问：**复现目标要区分逐位一致、指标近似一致和结论可重复。

</details>

<details>
<summary>20. 【八股深答】CUDA OOM 时为什么不能只把 batch 调小就结束？</summary>

**结论：**batch 是重要因素，但还要区分参数、优化器状态、激活、临时工作区、碎片和意外保留计算图。**机制：**若把带图的 loss Tensor 持续存入列表，图会跨 step 被引用，显存逐步增长；这与固定峰值过高不同。**工程影响：**先看峰值随 step 是否增长，再查 Shape、混合精度、梯度累积/checkpointing 和缓存统计。**误区：**`empty_cache()` 不会释放仍被 Tensor 引用的显存，也不是修复泄漏的根因。**追问：**reserved 高于 allocated 常是缓存分配器行为，不可直接判定泄漏。

</details>

<details>
<summary>21. 【八股深答】Notebook 的隐藏状态为什么是复现风险？</summary>

**结论：**单元格可乱序执行，内核中的变量可能来自已修改或删除的旧代码，磁盘文件不能完整表达当前状态。**机制：**Notebook 保存代码与输出快照，不保存一条可靠、线性的状态构建历史。**工程影响：**交付前重启内核并从上到下全部运行，把依赖、配置和数据路径显式化。**误区：**“我这里还能运行”不能证明别人或新进程能运行。**追问：**最可信的验收是干净环境中的自动化执行与产物检查。
